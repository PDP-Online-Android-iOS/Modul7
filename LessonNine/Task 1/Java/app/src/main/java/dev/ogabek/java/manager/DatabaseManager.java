...
public class DatabaseManager {
    ...
    public static void likeFeedPost(String uid, Post post) {
        database.collection(USER_PATH).document(uid).collection(FEED_PATH).document(post.getId()).update("isLiked", post.isLiked());
        if (uid.equals(post.getUid())) {
            database.collection(USER_PATH).document(uid).collection(POST_PATH).document(post.getId()).update("isLiked", post.isLiked());
        }
    }

    public static void loadLikedFeeds(String uid, DBPostsHandler handler) {
        Query reference = database.collection(USER_PATH).document(uid).collection(FEED_PATH).whereEqualTo("isLiked", true);
        reference.get().addOnCompleteListener(it -> {
            ArrayList<Post> posts = new ArrayList<>();
            if (it.isSuccessful()) {
                for (DocumentSnapshot document: it.getResult()) {
    ...
    public static void deletePost(Post post, DBPostHandler handler) {
        ...
        reference1.document(post.getId()).delete().addOnSuccessListener(it -> {
            CollectionReference reference2 = database.collection(USER_PATH).document(post.getUid()).collection(FEED_PATH);
            reference2.document(post.getId()).delete().addOnSuccessListener(it2 -> handler.onSuccess(post)).addOnFailureListener(handler::onError);
        }).addOnFailureListener(handler::onError);
    }
